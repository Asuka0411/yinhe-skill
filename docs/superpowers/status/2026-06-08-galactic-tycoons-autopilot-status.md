...
- `v0.1.69` 修复 07 当前位置误判：飞船详情中的 `Start flight` 是起飞按钮，不再被 `flight` 关键词误识别成 `transit`；跳过不在交易所的飞船时保留真实基地位置文本
...
- 已执行 `npm run release:local`，持久安装更新目标为 `v0.1.69`
- 已执行 `node scripts/hot-reload-chrome.js`，当前已打开的游戏页内存版本为 `v0.1.69`
