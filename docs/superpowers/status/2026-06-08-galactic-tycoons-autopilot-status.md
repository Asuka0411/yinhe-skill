...
截至 2026-06-10，当前脚本版本为 `v0.1.67`，`node --test tests/release-local.test.js tests/hot-reload-chrome.test.js tests/gt-autopilot.test.js` 的结果为 `133/133` 通过。
...
- `v0.1.67` 修复 06 错误校验空 wishlist 的问题：06 不再读取或校验 wishlist；购买后的 wishlist 被游戏清空是正常状态。若 05 保存的本轮购买清单不存在，06 会直接从当前 `Exchange Warehouse` 可见物资行读取可转移物资，按黑名单过滤后逐项装船
- 已增加回归测试：没有 05 缓存且 wishlist API 返回空时，06 仍从交易所仓库行读取 `Bioxene`、`Kryon` 等非黑名单物资并逐项转移
...
- 已执行 `npm run release:local`，持久安装更新目标为 `v0.1.67`
- 已执行 `node scripts/hot-reload-chrome.js`，当前已打开的游戏页内存版本为 `v0.1.67`
