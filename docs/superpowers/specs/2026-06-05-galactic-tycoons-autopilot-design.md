...
面板中的每一项物资配置都必须按完整条目保存，不能只保存 `id` 后再丢弃其它字段。

物资配置条目至少包含：
- `id`：游戏物资 ID
- `name`：当前脚本展示名
- `enabled`：该条配置是否启用
- `iconId`：游戏图标 symbol ID
- `iconHref`：完整 SVG atlas 引用

卖货白名单额外包含：
- `minAmount`：最小发货量

后续新增字段时，读取面板、标准化配置、保存配置三处必须同步保留该字段。除非用户明确要求重置，否则保存按钮不得因为 UI 重渲染或物资选择控件读值而丢弃已有配置字段。

### 物资图标规则

物资图标优先复用游戏前端的 SVG atlas。当前确认的 atlas 路径为 `/assets/atlas-_p6d2Xs0.svg`。

脚本保存 `iconId` 与 `iconHref` 两个字段：
- `iconId` 用于识别图标 symbol
- `iconHref` 用于 `<svg><use href="..."></use></svg>` 渲染

`gamedata.json` 当前不直接提供图标 URL，因此图标映射来自游戏前端 bundle 的物资枚举名。少数物资的展示名与图标枚举名不一致时，必须保留显式映射，例如 `Graphenium Wire (#80)` 的图标仍使用 `Superconductors`。
