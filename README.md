...
- `检查` 会先识别飞船当前位置，再分发执行：在基地则把指定货物卖到交易所，在交易所则把货物转移到飞船并发船回基地，运输中则等待到达基地；点击后立即写入日志与运行历史，读取失败也会记录失败原因而不是静默无响应
- 单基地、单飞船
...
- 补货回运原子功能面板已拆分完成，包含读取基地、清空 wishlist、检查飞船、创建 wishlist、购买、装船、补油修理、补修理包和油、发船；其中“打开交易所”和“读取 wishlist”作为购买步骤内部动作，不再单独展示为原子步骤
...
- `检查`：已在 `seed=sell`、`seed=transit`、`seed=resupply` 场景实跑验证，可识别飞船位置并分发——基地走卖货链、交易所走补货回运链、运输中只等待；点击后立即写入日志与运行历史
...
截至 `2026-06-16`，当前脚本版本为 `v0.1.71`，完整本地 Node 单测 `149/149` 通过。
...
06 `转移到飞船` 当前为可测试：进入交易所仓库后先读取当前真实选中的 `btnradio-whwt` 飞船货仓标签，并以该飞船作为本次装船目标；随后点击该目标飞船标签，再读取 `input[id^="btnradio-whwt"].checked` 对应的标签或 active 标签，核对当前真实选中飞船必须等于目标飞船。如果点击后仍选中其他飞船，立即失败并且不会点击任何物资转移按钮。核对通过后，脚本逐项点击当前 `Exchange Warehouse` 可见物资行的右箭头，并在每个转移弹窗里点击确认勾。06 不再校验 wishlist，也不会因为 wishlist 被 05 买空而失败；如果 05 保存的本轮购买清单仍在，则优先按缓存清单装船，否则直接读取交易所仓库可见物资行。`一键购买 wishlist` 展开面板内新增 `转移黑名单` 配置，默认保护 `Ship Repair Kit (#113)` 与 `Antimatter (#149)`；启用的黑名单物资不会被点击转移。`v0.1.67` 修复 06 空 wishlist 误判：没有 05 缓存时不再读 wishlist API，而是从交易所仓库行生成装船批次。

06 `转移到飞船` 已补充三类回归测试：旧快照显示 `200000 反物质-10` 仍在 `transit` 时会用实时选中货仓修正；交易所页基地上下文丢失、旧快照误指向 09 号但页面真实选中 10 号时，也会以 10 号货仓作为本次目标；没有 05 缓存且 wishlist API 返回空时，会从当前交易所仓库可见行装船而不是报 `wishlist 为空`。
...
`v0.1.69` 修复 07 当前位置日志误判：`Start flight` 是按钮文案，不代表飞船正在飞行；当前位置判断不再把普通 `flight` 字样当作 `transit`，跳过基地飞船时会保留真实基地位置文本。

底部旧原子按钮 `一键加油` 与 `一键修飞船` 已合并为 `一键加油、修理`，状态为 `可测试`，执行逻辑直接复用 07 `飞船补油修理`：逐艘处理当前交易所飞船，并按 `Refuel` 后 `Repair` 的顺序点击游戏真实控件。
